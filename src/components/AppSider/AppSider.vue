<template>
  <section class="flex">
    ...
    <div
      ...
      class="fixed inset-0 z-20 transition-opacity duration-1000 bg-black opacity-50 lg:hidden"
    ></div>
    <!-- End Backdrop -->

    <div
      :class="siderVisiable ? 'translate-x-0 ease-out' : '-translate-x-full ease-in'"
      class="fixed inset-y-0 left-0 z-30 w-64 overflow-y-auto transition duration-300 transform shadow bg-component lg:translate-x-0 lg:static lg:inset-0"
    >
      <AppSiderLogo />
...
export default defineComponent({
  ...
  setup(props) {
    ...
    const menuItems = reactive([
      ...
      { name: 'Account', label: '用户', icon: 'user', to: '/account' },
    ])
